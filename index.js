    ...
    for (j = 0; j < days.length; ++j) {
      ...
      if (!days[j].classList.contains('empty')) {
        const hours = parseFloat(
          days[j].querySelector('.tk-hours').textContent
        );
        // console.log('hours', hours)
        if (days[j].classList.contains('grad-purple')) {
          // console.log(`unbillable work ${hours} hours`)
          data[j].unbillable = Number(
            parseFloat(data[j].unbillable + hours).toFixed(1)
          );
        }
        if (days[j].classList.contains('grad-blue')) {
          // console.log(`billable work ${hours} hours`)
          data[j].billable = Number(
            parseFloat(data[j].billable + hours).toFixed(1)
          );
        }
      }
    }
...
`;
  ...
  function billablePerWeek(week) {
    ...
    for (i = 0; i < week.length; ++i) {
      ...
      total = Number(
        (total + (week[i].billable + week[i].unbillable)).toFixed(1)
      );
    }
